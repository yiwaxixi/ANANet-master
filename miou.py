...
def per_class_iu(hist):#分别为每个类别计算miou，hist的形状（n，n）
    ...
    smooth=1e-5
    return (np.diag(hist) +smooth)/ (hist.sum(1) + hist.sum(0) - np.diag(hist)+smooth)

def per_class_PA(hist):
    # 矩阵的对角线上的值组成的一维数组/矩阵的所有元素之和，返回值形状(n,)
    #per_class accuracy每一类的准确度
...
def compute_mIoU(gt_dir, pred_dir, png_name_list, num_classes, name_classes):  
    ...
    ## 1
    hist = np.zeros((num_classes, num_classes))#hist初始化全零，在这里的hist的形状是[19,19]
    ...
    for ind in range(len(gt_imgs)): 
        ...
        # 每计算10张就输出一下目前已计算的图片中所有类别平均的mIoU值sha
