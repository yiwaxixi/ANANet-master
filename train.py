...
from module.ANANet_training import CE_Loss, Dice_loss
from utils.metrics import f_score
from torch.utils.data import DataLoader
from utils.dataloader import zywnet_dataset_collate, ZYWnetDataset
import warnings
...
def fit_one_epoch(net, epoch, epoch_size, epoch_size_val, gen, genval, Epoch, cuda, aux_branch):
    ...
    with tqdm(total=epoch_size, desc=f'Epoch {epoch + 1}/{Epoch}', postfix=dict, mininterval=0.3) as pbar:
        for iteration, batch in enumerate(gen):
            ...
            if aux_branch:
                ...
                aux_outputs, outputs,aux_outputs2 = net(imgs)
                aux_loss = CE_Loss(aux_outputs, pngs, num_classes=NUM_CLASSES)
                aux_loss2 = CE_Loss(aux_outputs2, pngs, num_classes=NUM_CLASSES)
                main_loss = CE_Loss(outputs, pngs, num_classes=NUM_CLASSES)
                loss = aux_loss*(0.6) + main_loss + aux_loss2*(0.4)
        ...
        for iteration, batch in enumerate(genval):
            ...
            with torch.no_grad():
                ...
                if aux_branch:
                    ...
                    aux_loss2 = CE_Loss(aux_outputs2, pngs, num_classes=NUM_CLASSES)
                    main_loss = CE_Loss(outputs, pngs, num_classes=NUM_CLASSES)
                    val_loss  = main_loss + aux_loss*(0.6) +aux_loss2*(0.4)
                    if dice_loss:
                        aux_dice = Dice_loss(aux_outputs, labels)
                        main_dice = Dice_loss(outputs, labels)
                        val_loss = main_dice + val_loss + aux_dice
    ...
    torch.save(model.state_dict(), 'logs_/Epoch%d-Total_Loss%.4f-Val_Loss%.4f.pth' % (
...
if __name__ == "__main__":
    ...
    # ---------------------------------------------------------------------#
    dice_loss = False  # 是否使用dice_loss
    # -------------------------------#
    #   主干网络预训练权重的使用
    #   mobilenet和resnet50
    # -------------------------------#
    pretrained = False  # 是否预训练，False:不预训练，True:预训练
    backbone = "resnet50"
    # ---------------------#
    #   是否使用辅助分支
    #   会占用大量显存
    # ---------------------#
    aux_branch = True  # 是否使用辅助分支
    # ---------------------#
    #   下采样的倍数
    #   8和16
    # ---------------------#
    ...
    model = ANANet(num_classes=NUM_CLASSES, backbone=backbone,
                   downsample_factor=downsample_factor, pretrained=pretrained, aux_branch=aux_branch).train()
    model_path = r"model_data/E8-83.42.pth"
...
#        lr_scheduler = optim.lr_scheduler.ExponentialLR(optimizer, gamma=0.9, last_epoch=-1)
        ...
        gen = DataLoader(train_dataset, batch_size=Batch_size, num_workers=3, pin_memory=True,
...
#        for param in model.master_branch.parameters():
#            param.requires_grad = False
...
"""
    ...
    if True:
        ...
        train_dataset = ZYWnetDataset(train_lines, inputs_size, NUM_CLASSES, True)
        val_dataset = ZYWnetDataset(val_lines, inputs_size, NUM_CLASSES, False)
        gen = DataLoader(train_dataset, batch_size=Batch_size, num_workers=3, pin_memory=True,
                         drop_last=True, collate_fn=zywnet_dataset_collate)  # num_workers=4使用多线程加载数据
        gen_val = DataLoader(val_dataset, batch_size=Batch_size, num_workers=2, pin_memory=True,
                             drop_last=True, collate_fn=zywnet_dataset_collate)
