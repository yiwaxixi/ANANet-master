...
#getitem函数，getitem接收一个index，然后返回图片数据和标签，这个index通常指的是一个list的index
    #这个list的每个元素就包含了图片的数据的路径和标签信息
...
def zywnet_dataset_collate(batch):
    images = []
    pngs = []
    seg_labels = []
